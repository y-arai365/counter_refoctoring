...
class TIS_GrabberDLL(object):
    if sys.maxsize > 2 ** 32:
        main_dir = os.path.dirname(os.path.abspath("__main__"))  # main.pyの存在するディレクトリ
        dll_dir = os.path.dirname(os.path.abspath(__file__))  # tisgrabber.pyとDLLは同じディレクトリに存在する
        os.chdir(dll_dir)  # カレントディレクトリを実行時の場所からDLLがある場所に移動
        __tisgrabber = C.windll.LoadLibrary("tisgrabber_x64.dll")
        os.chdir(main_dir)  # 元の場所に戻る
    else:
    ...
    GetPropertyAbsoluteValue.argtypes = (GrabberHandlePtr,
                                         ...
                                         C.POINTER(C.c_float),)
    """↓追加↓"""
    GetAutoProperty = __tisgrabber.IC_GetAutoCameraProperty
    GetAutoProperty.argtypes = [GrabberHandlePtr, C.c_int, C.POINTER(C.c_int)]
    GetAutoProperty.restype = C.c_int

    SetAutoProperty = __tisgrabber.IC_EnableAutoCameraProperty
    SetAutoProperty.argtypes = [GrabberHandlePtr, C.c_int, C.c_int]
    SetAutoProperty.restype = C.c_int
    """↑追加↑"""
...
class TIS_CAM(object):
    ...
    def GetAutoExposureProperty(self):
        """
        追加。
        reference: https://github.com/TheImagingSource/IC-Imaging-Control-Samples/issues/9
                 : https://github.com/AndrewGYork/msim/blob/master/hardware_control/theimagingsource.py
        :return:
        result         : 取得成功したかどうか
        autoexposure_is: 1/0
        """
        autoexposure = C.c_int(777)
        result = TIS_GrabberDLL.GetAutoProperty(self._handle, 4, autoexposure)
        autoexposure_is = autoexposure.value
        return result, autoexposure_is

    def SetAutoExposureProperty(self, autoexposure_is):
        """
        追加。
        reference: https://github.com/TheImagingSource/IC-Imaging-Control-Samples/issues/9
                 : https://github.com/AndrewGYork/msim/blob/master/hardware_control/theimagingsource.py
        :param autoexposure_is: 1/0
        :return: 設定成功したかどうか
        """
        error = TIS_GrabberDLL.SetAutoProperty(self._handle, 4, autoexposure_is)
        return error
